...
## 概要
本モデルは28×28のグレースケール画像を入力とし，中間層で4次元まで情報を圧縮した後，元の画像を再構築するオートエンコーダモデルである．
...
Encoder部は入力画像(784次元)を段階的に削減して4次元の潜在変数に変換し，Decoder部はそれを元の784次元に再構築する．
...
- 出典: [＠IT - PyTorchでオートエンコーダーによる画像生成をしてみよう](https://atmarkit.itmedia.co.jp/ait/articles/2007/10/news024.html, by Shinji Kawasaki)
...
#### 1. 入力データの定義
MNIST画像は $28 \times 28$ の2次元データだが，本モデルでは全結合層に入力するため，1次元の列ベクトル $\boldsymbol{x}$ に平坦化して扱う．
